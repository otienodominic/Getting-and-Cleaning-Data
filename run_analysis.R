## This assignment helps to demonstrate my ability to to collect, work with, and clean a data set. The steps taken below help to solve
...
if(!file.exists("./manyien")){dir.create("./manyien")}
getwd()
[1] "C:/Users/Dominic/Documents/data"
setwd("manyien")

# Get file url and download data

url <- "https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
the Getting and cleaning data assignment.

## Create one R script called run_analysis.R that does the following:
## 1. Merges the training and the test sets to create one data set.
ucData<- download.file(url, destfile= "./Dataset.zip")
...
# Read the file for Subject data
dataSubjectTrain <- tbl_df(read.table(file.path(filesPath, "train", "subject_train.txt")))
dataSubjectTest  <- tbl_df(read.table(file.path(filesPath, "test" , "subject_test.txt" )))

# Read the file for Activity data
...
# Combine the Subject data and reset the names of the variables
alldataSubject <- rbind(dataSubjectTrain, dataSubjectTest)
setnames(alldataSubject, "V1", "subject")

# Combine the Activity data and reset the names of the variables
alldataActivity<- rbind(dataActivityTrain, dataActivityTest)
setnames(alldataActivity, "V1", "activityNum") 

# Combine the Train and Test data and reset the names of the variables
dataTable <- rbind(dataTrain, dataTest)

# Name the variables according to the features
dataFeatures <- tbl_df(read.table(file.path(filesPath, "features.txt")))
setnames(dataFeatures, names(dataFeatures), c("featureNum", "featureName"))
colnames(dataTable) <- dataFeatures$featureName

# Give column names to activity lables
...
# Write the tidy data into a text file named "TidyData" 
